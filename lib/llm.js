...
// GPT-5 以降は Chat Completions の仕様が変わっている:
//   - max_tokens は不可 → max_completion_tokens を使う
//   - temperature は既定(1)のみ。他の値を渡すと 400 になるため送らない
// これらを吸収するため、モデル名で分岐する。
function isOpenAINewGen(model) {
...
async function callOpenAI({ apiKey, model, system, prompt, maxTokens }) {
  ...
  try {
    const res = await client.chat.completions.create({
      model,
      ...(newGen ? { max_completion_tokens: maxTokens } : { max_tokens: maxTokens, temperature: 0.7 }),
...
async function streamOpenAI({ apiKey, model, system, messages, maxTokens, onDelta }) {
  ...
  try {
    const stream = await client.chat.completions.create({
      model,
      ...(newGen ? { max_completion_tokens: maxTokens } : { max_tokens: maxTokens, temperature: 0.8 }),
      stream: true,
