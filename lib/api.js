...
export function generateWord({ provider, apiKey, word, count, level, model }) {
  return postJSON("/api/generate/word", { provider, apiKey, word, count, level, model })
}
...
// 英文1文の和訳。context=直近の会話（文脈把握用・任意）。戻り: { ja }
export function translateSentence({ provider, apiKey, model, en, context }) {
  return postJSON("/api/translate", { provider, apiKey, model, en, context })
}
...
// ユーザーの入力文を自然な英語に変換（日本語→英訳／英語→自然な英語）。context=直近の会話（任意）。戻り: { en }
export function refineToEnglish({ provider, apiKey, model, text, context }) {
  return postJSON("/api/refine", { provider, apiKey, model, text, context })
}

// チャット（ストリーミング）。onDelta(chunk, whole) を逐次呼ぶ。戻り値は全文。
export async function streamChat({ provider, apiKey, model, messages, level }, onDelta) {
  const res = await fetch("/api/chat", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ provider, apiKey, model, messages, level }),
  })
  if (!res.ok || !res.body) {
    const data = await res.json().catch(() => ({}))
